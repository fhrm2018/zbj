...
function convertMsgtoHtml(msg) {
    ...
    if (cmdObj.groupId == 3) {
        if (cmdObj.isAt == 1) {
            ...
            return webim.Tool.formatHtml2Text("<image src='../static/images/whosaid.png' /><span class='msgName ml10 mr10 greenFont'>" + text[0] + "</span><span style='display:inline-block;color: #d51414;font-size:16px;font-weight:bold;border-radius:5px; background-color: #fbe1e1;padding: 4px 10px;'> " + other + "</span>");
        } else {
            return webim.Tool.formatHtml2Text("<span style='display:inline-block;color: #d51414;font-size: 16px;font-weight:bold;border-radius: 5px;background-color: #fbe1e1;padding: 4px 10px;'>" + html + "</span>");
        }
    } else if (cmdObj.groupId == 4) {
        return webim.Tool.formatHtml2Text("<span style='color:#ffcd56;'>" + html + "</span>");
    } else {
    ...
    if (cmdJson.isSpecial == 0) {
        if (cmdJson.type == 0) {
            htmls += [
                ...
                // '<span class="msgTime">' + getNowFormatDate(cmdJson.sendTime) + '</span>',
                getHeaderHtml(cmdJson.groupId, cmdJson.level, cmdJson.small),
                '<span class="msgName ml5 ' + (cmdJson.groupId == 1 ? 'greenFont' : '') + ' ' + (cmdJson.groupId == 3 ? 'redFont' : '') + ' ' + (cmdJson.groupId == 4 ? 'redFont' : '') + ' ' + (cmdJson.groupId == 5 ? 'greenFont' : '') + '">' + cmdJson.postNickName + ' :</span>',
        ...
        if (cmdJson.isImg == 1) {//如果发送的是图片, 直接去附加参数里的图片地址
            htmls += [
                ...
                // '<span class="msgTime">' + getNowFormatDate(cmdJson.sendTime) + '</span>',
                getHeaderHtml(cmdJson.groupId, cmdJson.level, cmdJson.small),
                '<span class="msgName ml5 ' + (cmdJson.groupId == 1 ? 'greenFont' : '') + ' ' + (cmdJson.groupId == 3 ? 'yellowFont' : '') + ' ' + (cmdJson.groupId == 4 ? 'yellowFont' : '') + ' ' + (cmdJson.groupId == 5 ? 'greenFont' : '') + '">' + cmdJson.postNickName + ' :</span>',
                '<div class="ilblock posRel ml10">',
