...
import com.qiyou.dhlive.prd.live.vo.ItemVO;
import com.yaozhong.framework.base.common.utils.EmptyUtil;
import com.yaozhong.framework.base.common.utils.ExcelUtil;
...
public class LiveController extends ResourceBaseController {
    ...
    public DataResponse exportChatMsg(HttpServletResponse response) {
        ...
        for (int i = 0; i < listJson.size(); i++) {
            ...
            ext.setSendTime(TimeConverterUtil.utc2Local(base.getSendTime(), "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", "yyyy-MM-dd HH:mm:ss"));
            if (EmptyUtil.isNotEmpty(base.getAuditUid())) {
                String auditUserName = this.manageInfoService.findById(Integer.parseInt(base.getAuditUid())).getUserNickName();
                ext.setAuditUserName(auditUserName);
            } else {
                ext.setAuditUserName("");
            }
            ext.setLevel(base.getLevel());
            ext.setAuditTime(TimeConverterUtil.utc2Local(base.getAuditTime(), "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", "yyyy-MM-dd HH:mm:ss"));
        ...
        Ordering<ExtChatMsgVO> sortByStatus = Ordering.natural().onResultOf(
                new Function<ExtChatMsgVO, String>() {
