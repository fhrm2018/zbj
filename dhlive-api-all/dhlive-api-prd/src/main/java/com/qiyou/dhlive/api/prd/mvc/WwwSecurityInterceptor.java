...
public class WwwSecurityInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (!isAjaxRequest(request)) {
				...
				if (this.checkIpVisitService.checkCanVisit(ip, url)) {
					if (EmptyUtil.isEmpty(userLogin)) {
						...
						if (EmptyUtil.isEmpty(userId)) {
							...
							userId = getCookiesValue(request, Constants.MANAGE_USER_ID);
							if(EmptyUtil.isEmpty(userId)) {
								...
								UserInfoDTO userDto = this.baseCacheService.getUserManager(Integer.parseInt(userId));
								if(EmptyUtil.isNotEmpty(userDto)) {
									user = MyBeanUtils.copyBean(userDto, UserInfo.class);
								}
							}
						}else {
							UserInfoDTO userDto = this.baseCacheService.getUserVip(Integer.parseInt(userId));
							if(EmptyUtil.isNotEmpty(userDto)) {
								user = MyBeanUtils.copyBean(userDto, UserInfo.class);
							}
						}
						if (EmptyUtil.isEmpty(user)) {
							/*
							 * user =
							 * this.userInfoService.createNewGuestUser(AddressUtils.getIpAddrFromRequest(
							 * request), utmSource); user.setUserId(user.getUserId()); UserSession
							 * userSession = HttpSessionTool.createUserSession(user);
							 * HttpSessionTool.doLoginedUser(httpSession, userSession); Cookie userIdCookie
							 * = new Cookie(Constants.USER_ID, user.getUserId().toString());
						...
						/*
						 * Integer userId = userLogin.getUserId(); if(EmptyUtil.isNotEmpty(userId)) {
						 * UserInfo user = this.userInfoService.findById(userId);
						 * if(EmptyUtil.isNotEmpty(user)) { UserInfo record = new UserInfo();
						 * record.setLastLoginIp(ip); record.setUserId(userId);
						 * record.setLastLoginTime(new Date());
						 * this.userInfoService.modifyEntity(record); } }
						 */
					}
			...
			// baseLog.info(LogFormatUtil.getActionFormat("拦截器获取ajax:" + ip + ",url:" +
			// url));
			if (unSession == null) {
				if (EmptyUtil.isEmpty(userLogin)) {
					String userId = "";
